...
export class UserService {
  ...
  // ?nat=us,dk,fr,gb
  // ?page=3&results=10
  private readonly urlGetUser: string = 'https://randomuser.me/api/?page=1&results=10'

  constructor(private http: HttpClient) { }

  getUser(page=1,pageResult=10,filterObject:any): Observable<any> {
    // gender?:string,nat?:string[]
    let params = new HttpParams();
    let gender =filterObject.gender
    let nat =filterObject.nat as Array<string>
    if(nat){
      nat= nat?.map(item=>item?.toLowerCase())
    }
    params = params.append('gender', gender);
    // params = params.append('nat', nat);
   
    let url =`https://randomuser.me/api/?page=${page}&results=${pageResult}&exc=login&seed=abc&nat=${nat}`
